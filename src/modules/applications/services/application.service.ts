...
export class ApplicationService {
  ...
  async setup(_p: SetupAppResquestDto): Promise<SetupAppResponseDto> {
    ...
    switch (_p.app.id) {
      case APP_ID.ENTUKSA_CLIENT:
        ...
        blocks.push({
          data: localityOffers.slice(0, 3) as ShopOffer[],
          type: 'offers-slider'
        });
        blocks.push({
          data: 'Ofertas',
        ...
        blocks.push({
          data: localityOffers.slice(3, localityOffers.length) as ShopOffer[],
          type: 'offers-group',
          config: { displayDense: true }
  ...
  /**
   * extractStore
   * @param _store ShopStore
   * @returns DeepPartial<ShopStore & { imageId: number }>
   */
  private extractStore(_store: ShopStore): DeepPartial<ShopStore & { imageId: number }> {
    return {
      id: _store.id,
      createdAt: _store.createdAt,
      description: _store.description,
      imageId: _store.image.id,
      open: _store.open,
